  ...
  def show
    if request.user_agent =~ /MicroMessenger/ # visit from weixin
      if current_user # step 3
        ...
        redirect_to "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx0e604319f74644b3&redirect_uri=http%3A%2F%2F#{Weixin.callback_url}/interviews/#{params[:id]}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect"
      end
    else
      @interview = Interview.find(params[:id])
    end
